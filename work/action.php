...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'insert')
	{
		...
		simpleLog('action.php POST action == \'insert\' - calls curl_init '.$api_url.' ');
	...
	if($_POST["action"] == 'fetch_single')
	{
		...
		simpleLog('action.php POST action == \'fetch_single\' - calls curl_init '.$api_url.' ');
		$client = curl_init($api_url);
		curl_setopt($client, CURLOPT_RETURNTRANSFER, true);
		$response = curl_exec($client);
		echo $response;
	...
	if($_POST["action"] == 'update')
	{
		...
		simpleLog('action.php POST action == \'update\' - calls curl_init('.$api_url.')');
		$client = curl_init($api_url);
		curl_setopt($client, CURLOPT_POST, true);
		curl_setopt($client, CURLOPT_POSTFIELDS, $form_data);
		curl_setopt($client, CURLOPT_RETURNTRANSFER, true);
	...
	if($_POST["action"] == 'delete')
	{
		...
		simpleLog('action.php POST action == \'delete\' - calls curl_init('.$api_url.')');
		$client = curl_init($api_url);
		curl_setopt($client, CURLOPT_RETURNTRANSFER, true);
		$response = curl_exec($client);
		echo $response;
